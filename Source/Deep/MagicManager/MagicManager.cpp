...
{
	const int32 Index = int32(Slot);

	if (!GetOwner()) return;
	if (Index < 0 || Index > 2) return;

	if (!Magics.IsValidIndex(Index)) return;

	if (!Magics[Index])
	{
		UE_LOG(LogTemp, Warning, TEXT("No magic in slot %d"), Index);
		return;
	}

	if (!MagicHandles[Index].IsValid())
	{
		UE_LOG(LogTemp, Warning, TEXT("Invalid MagicHandle in slot %d"), Index);
		return;
	}
	...
	if (!ASC->FindAbilitySpecFromHandle(MagicHandles[Index]))
	{
		UE_LOG(LogTemp, Error, TEXT("Handle valid but AbilitySpec not found. Slot: %d"), Index);
		return;
	}

	ASC->TargetCancel();

	const bool bActivated = ASC->TryActivateAbility(MagicHandles[Index]);

	if (!bActivated)
	{
		UE_LOG(LogTemp, Warning, TEXT("Failed to activate magic ability. Slot: %d"), Index);
	}
}
